function [longbars, trasvbars]=MTAsimdesignEXT(year,dimpier,type,Nsd,Msd_x,Msd_y,Vsd_x,Vsd_y,sigmac_r,sigmas_r, varargin)
...
%% Type of section and geometry definition

if strcmp(type,'circ')
    if length(dimpier)==1
        D=dimpier;
    else
        warning('Error in input dimension and type')
    end
...
if strcmp(type,'circ')
    [longbars, ~]=LreinfdesignMTA(year,Lx,Ly,Nsd,Msd_y,Msd_x,sigmac_adm,sigmas_adm,cover,n);   
elseif strcmp(type,'rect') 
